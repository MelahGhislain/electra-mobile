...
//   // Emits normalised amplitude values in [0.0, 1.0] at ~60 fps.
//   // Driven by the same AudioRecorder that captures audio, so no second mic
//   // consumer is needed.
...
//   // ── Audio capture + amplitude polling ─────────────────────────────────────
...
//     // Poll amplitude from the same recorder at ~60 fps.
//     // getAmplitude() is non-blocking and does NOT open a second mic stream.
//     _amplitudeTimer =
//         Timer.periodic(const Duration(milliseconds: 16), (_) async {
//       try {
//         final amp = await _recorder.getAmplitude();
//         if (!_amplitudeController.isClosed) {
//           // amp.current is in dBFS: typically -60 (silence) to 0 (peak).
//           // Normalise to [0.0, 1.0].
//           const double minDb = -60.0;
//           final normalised =
//               ((amp.current - minDb) / minDb.abs()).clamp(0.0, 1.0);
//           _amplitudeController.add(normalised);
//         }
//       } catch (_) {
//         // Recorder may not be active yet — ignore.
//       }
//     });
//   }
...
//     // 2. Stop amplitude polling.
//     _amplitudeTimer?.cancel();
//     _amplitudeTimer = null;

//     // 3. Stop mic capture — BE no longer needs audio.
//     await _audioSubscription?.cancel();
//     _audioSubscription = null;
//     await _recorder.stop();

//     // 4. Do NOT close the socket — BE will still send
